...
| **Standoff ≥300 mm from a WALL** | Drive at an obstacle, `/scan` healthy | Settled bumper clearance ≥ 0.30 m | ⚠️ **PASSED BY 15 mm, 2026-08-11, `/scan`-only** — `tools/collision_standoff_test.py`, n=1. Fired at **0.347 m** (vs `stop_distance` 0.35 — accurate to 3 mm), coast **0.034 m**, settled **0.315 m**. ⛔ **NOT tape-verified** — the rover was repositioned before the gap was measured, so the only witness is the sensor whose scale is itself in question. **15 mm of margin is not a comfortable pass**; repeat with a tape before relying on it |
...
| **`/odom` distance vs `/scan` closure** | 🔴 **17% DISAGREEMENT, UNRESOLVED** | Same motion, two rulers: `/scan` closure **1.543 m** (a *differential* measurement, independent of the `front_overhang` constant) vs `/odom` pose displacement **1.288 m**. Odom reads **17% low** — uncomfortably close to the *old* 19% scale error that `erpm_to_ms → 0.003900` was supposed to have cut to ~2.4%. Either odom is off at low speed or depth ranging has its own scale error. **Settle it with `tools/odom_scale_measure.py`** — it computes the scale from raw `esc_status`, so it does not depend on the value under suspicion. This bears on every distance the rover believes it has driven, mapping included |
