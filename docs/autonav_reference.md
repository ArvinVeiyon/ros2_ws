...
| **Standoff ≥300 mm from a WALL** | Drive at an obstacle, `/scan` healthy | Settled bumper clearance ≥ 0.30 m | ⬜ **untested at current parameters.** The 2026-07-22 armed wall test predates the correction of `stop_distance` to 0.35 m at the bumper. `tools/collision_standoff_test.py` written and compile-checked. **This is a different question from S2** — S2 runs a deliberately CLEAR corridor and measures latency; this measures distance |
...
| **Stopping DISTANCE from a wall (≥300 mm)** | ⬜ **NOT MEASURED — and no distance in this file is observed** | Every distance quoted after the S2 runs is **arithmetic from the commanded speed**: those runs never logged `/odom`. Derived, assuming coast time is speed-independent (**unmeasured**): ~0.33 m standoff at 0.08 m/s, ~0.31 m at 0.15, **~0.28 m at 0.30 m/s — i.e. the ≥300 mm requirement runs out near 0.2–0.25 m/s**. ⛔ Do not plan a speed ladder on this until `tools/collision_standoff_test.py` has been run |
