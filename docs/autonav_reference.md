...
| Drive | 4× VESC over DroneCAN, skid-steer | addresses 10–13; ESCs doze at rest. **Motors: Yalu 6" hub, 24 V 250 W — DIRECT DRIVE, no gearbox.** Wheel **155 mm** measured (6" nominal runs *over* nominal). 🔑 **ADDRESS MAP, established 2026-08-13 by spinning each wheel by hand and reading which address moved: `10` = right-front · `11` = front-left · `13` = rear-left · `12` = right-rear (by elimination, NOT directly confirmed).** Sides are `L={11,13}` `R={10,12}`; **addr 10 reports INVERTED** (`SIGN={10:-1.0}`) and that was re-confirmed on 2026-08-13 — its raw integral came out negative |
...
| `/scan` range scale | **0.9845** (−1.55%) | ✅ **Two-point tape calibration 2026-08-12.** Tape 1.960 m → `/scan` 1.932; tape 0.345 m → `/scan` 0.342. Fitting `scan = a·tape + b` gives **a = 0.9845, b = +2.3 mm** — i.e. a pure scale error with **no meaningful offset**. 🔑 Two consequences: **`front_overhang` 0.337 m is confirmed to within 2 mm** (a wrong overhang would appear as an offset), and `/scan` reads **short**, so the reflex triggers slightly **early** — the safe direction. Apply the 1.55% when comparing `/scan` travel against any other ruler. ✅ **RE-CONFIRMED 2026-08-13 at a THIRD range by operator tape**: parked 1.973 m from the bumper, the straight-ahead ray read **2.273 m against 2.274 predicted — 1 mm.** A rival free fit (scale 0.9573, overhang 0.377) that matched two closer points equally well was **refuted by 26 mm** at this range. Both constants stand unchanged. ⚠️ **Two traps found while measuring, both of which produced a wrong answer first:** (1) the operator taped to a **14 mm rib at the base of the wall** — but the `/scan` band sits **0.23–0.38 m above the floor** at ~1.5 m (cam_z 0.305, ±2.79°), so it ranges the **wall FACE and cannot see a skirting rib**; tape the face at ~0.30 m up. (2) **A person standing beside the rover while taping appears in the ±20° cone** — one 2° bin at +18° read 1.751 m against a 2.39 m wall and dragged the sector minimum *below* the reading from a closer position. 🔑 **Always profile range-vs-angle before trusting a sector minimum; a single anomalous bin is a body, not a wall** |
...
|---|---|---|
| `erpm_to_ms` | **0.003900** | Wall-referenced, 5 powered runs both directions, 0.146–0.364 m/s. Odometry over-reported ground distance by **18.8%** (mean 1.188, sd 0.029). Residual after correction **~2.4%**. ✅ **INDEPENDENTLY CORROBORATED 2026-08-12**: a tape across `house_map_v4` agreed on the long axis to within a few percent — **3.200 m mapped vs 3.1 m taped**, i.e. the map reads **+3.2% large**, just above the ~2.4% residual quoted here and nowhere near a gross error. The caveat *"includes mean slip — re-measure on a different floor"* stands, but the constant has now survived a ruler sharing none of its assumptions. ⚠️ Note the geometric value is 0.004633 and a 2.13 m tape drive gave 0.004463 — **both disagree with 0.003900 by ~16–19%, and the map says 0.003900 is the right one.** Do not "correct" it toward the geometric figure without repeating the map check. ✅✅ **VINDICATED 2026-08-13 — KEEP 0.003900.** The corrected slip-free constant is **0.004058**, so 0.003900 implies **4.0% slip**, which is physical; the old 0.004633 implied 18.8%, which was not. 🔴 **The ~20% crawl under-read is NOT a scale error — it is the ESC zero-dropout corrupting `∫ERPM·dt`** (§10). Scale contributes ~0. ⛔ **This closes the "calibrate `erpm_to_ms`" line of work: do not re-open it.** |
| ~~`erpm_to_ms` 0.004633~~ | **superseded 2026-08-09 · inputs REFUTED 2026-08-13** | The slip-free eRPM→*wheel rotation* figure from a hand push over 5 counted revolutions. Correct as a description of the drivetrain, **wrong for odometry**, which needs ground distance. Itself replaced an assumed-geometry value that was 12.2× too small. Full history in `config/rover_odometry.yaml`. 🔴 **BOTH INPUTS TO IT ARE NOW MEASURED WRONG** (2026-08-13): the encoder figure **103.34 → 119.58 ERPM-s/rev (+15.7%)** and the wheel **0.1524 → 0.155 m (+1.7%)**. The corrected slip-free value is **0.004072**. ⛔ **0.004633 was never a valid "geometric bound" — delete that idea wherever it appears.** The crawl scale "exceeding" it was the bound being wrong, not the measurement. |
| **`erpm_to_ms` slip-free** | **0.004058** (R=2) | 🔑 **THE DRIVETRAIN CONSTANT, 2026-08-13.** `π × 0.155 / 120`. **Six independent hand-rotation runs** across three sessions and four wheels: 103.34 · 112.80 · 133.30 · 132.86 · 114.46 · 120.71 → **pooled 119.58 ERPM-s/rev, sd 9.9%**, i.e. **R = 1.9930, within 0.35% of exactly 2**. ⚠️ **This is a SLIP-FREE constant (wheel rolling), NOT ground distance — do not compare it directly to the configured 0.003900**, which is a ground-distance constant. Doing exactly that produced a bogus "agrees to 0.4%" claim on 2026-08-13. **Together they imply 4.0% slip under power** — against the 18.8% the old wrong inputs forced, and far more physical. ⚠️ **Individual runs scatter ±10% and the error is the HUMAN REVOLUTION COUNT, not the hardware** — never trust one run. **Wheel measured 155 mm (6" nominal runs OVER nominal, it is not worn under).** |
| **R = 2** (motor revs per wheel rev) | **`si_motor_poles` is set to HALF the true pole count** | 🔴 **The rover is DIRECT-DRIVE hub motors (Yalu 6", 24 V, 250 W) — there is NO gearbox, so R must physically be 1.** It measures 2. Cause: `libcanard/canard_driver.c:494` sends `status.rpm = mc_interface_get_rpm() / (si_motor_poles/2.0)`, so **the CAN value is MOTOR RPM — pole-corrected, NOT gear-corrected.** Entering the pole-**pair** count into `si_motor_poles` (which wants the pole **count**) gives exactly 2× regardless of the motor. ⛔ **`config/rover_odometry.yaml` states the ratio is `pole_pairs × gear_ratio` — that model is WRONG**; pole pairs are already divided out in firmware. → §10 for the linked-pair hazard |
...
|---|---|---|
| 🔴🔴 **`RO_ACCEL_LIM`/`RO_DECEL_LIM` slew the MANUAL stick — releasing it no longer stops the rover** *(caused a HARD WALL HIT 2026-08-14; REVERTED to −1 the same day)* | Under RC Manual, the rover keeps driving under power after the stick is centred. Nothing errors, the stick feels normal, and **the collision reflex does not intervene because it gates AutoNav setpoints, not the operator** | ⛔ **KEEP BOTH AT −1.** Verified in `~/PX4-Autopilot` `a52c38b07d`: `control_mode.cpp:55` sets `flag_control_allocation_enabled` in `NAVIGATION_STATE_MANUAL` ⇒ `RoverDifferential.cpp:154` runs `DifferentialActControl` ⇒ `DifferentialActControl.cpp:75` slew-limits the **raw stick throttle** via `RoverControl::throttleControl`. Decel slew = **`RO_DECEL_LIM / RO_MAX_THR_SPEED`** = 0.5/0.60 = 0.833 /s ⇒ **1.2 s of powered travel after "stop"** (~0.5 m at the ~0.9 m/s this rover reaches) **plus** the ~0.30 m coast ≈ **3× the manual stopping distance**. At −1, `RoverControl.cpp:70` passes the stick straight to the motors and the cut is instant. 🔑 **A rover param is scoped by the `flag_control_*_enabled` bits, NOT by its description** — the firmware doc says "auto modes" and that is incomplete. 🔑 **"Not verified on the vehicle" ≠ "not active".** → `setup_manual` §A7 · `px4_param_audit.md` §4 |
| 🔴 **Speed commands are not honoured — the vehicle runs far faster than asked** | Commanded vs measured, 2026-08-12/13: **0.05 → 0.140 · 0.06 → 0.146 · 0.25 → ~0.9 m/s.** The ratio *grows* with the command, and there is a floor near **0.14 m/s** — the rover cannot be asked to go slower. Nothing errors; the setpoint is emitted correctly and the wheels simply spin faster than commanded. ⚠️ **The 0.05 figure was recorded as "~0.11" until 2026-08-13 — that was `/odom`, which under-reads.** The true value against the tape-validated bumper ruler is **0.140 m/s, 2.80×**. 🔬 **The speed CONTROLLER is exonerated** (`tools/speed_command_test.py`, 3301 samples, 2026-08-12): throttle **never saturated**, slope **−0.0014/s** against the **+0.0050/s** an integral windup would produce, and the loop cut throttle to **0.032 against a 0.083 feedforward** — it saw the overspeed and fought it. ⛔ **Do NOT re-propose integral windup, a dead EKF feedback, or `RO_SPEED_I=0` as the fix; all three are disproven.** Look DOWNSTREAM of the speed controller (allocation / ESC / `erpm_to_ms`) | ⛔ **SAFETY-CRITICAL, not tuning.** Stopping distance is set by the coast, so the vehicle cannot be commanded to stay inside the speed band its own stopping distance requires. **This is what drove the rover into a wall on 2026-08-12**: a run sized from a 0.25 m/s command executed at ~0.9 m/s and stopped 0.020 m short. ⛔ **Gate every moving test on measured `/odom` speed, never on the command** (`--max-measured-speed`). Suspect the PX4 rover-module speed scaling (`RO_*`) |
| 🔴 **THE 0.14 m/s FLOOR IS `RO_SPEED_TH` — MECHANISM FOUND 2026-08-13** | The rover cannot be commanded below ~0.14 m/s. **Cause, from the flashed firmware:** `DifferentialSpeedControl.cpp:105` — `_vehicle_speed = norm > RO_SPEED_TH ? ... : 0.f` — **forces the feedback speed to EXACTLY ZERO below `RO_SPEED_TH` (0.10 m/s).** The loop then believes the rover is stopped and re-accelerates. Proven from the 3329-sample run `speed_cmd_20260812_231229.json`: EKF speed **pinned in 0.096–0.118** straddling the 0.10 boundary; throttle modulating **0.0258 (feedback live) ↔ 0.0531 (feedback zeroed) = 2.06×**, switching precisely on that boundary. Solving `thr = FF + P·(sp−v_fb) + I` for the integral predicts the live branch to **4 decimals** (0.0258 vs 0.0258) and the zeroed branch to **1%** (0.0779 vs peak 0.0794). ⚠️ **`/odom` under-reads ~23% at crawl, so a believed 0.10 floor is a TRUE ~0.136 m/s.** | **You cannot command below `RO_SPEED_TH` — it is structural, not tuning.** Lowering it does not help on its own: the feedback below it is corrupted by the ESC dropout anyway. ⛔ **Do NOT conclude `RO_MAX_THR_SPEED` is wrong from this data** — see the row below |
| ⛔ **The plant gain CANNOT be identified from a closed-loop run** *(method trap, cost a wrong answer 2026-08-13)* | Dividing mean speed by mean throttle in the closed-loop speed run gives "G = 4.47 m/s per unit throttle vs `RO_MAX_THR_SPEED` 0.60, understated 7.4×". **It is an artefact.** Lag correlation on the same run: all `|r| ≤ 0.28`, with a **negative simultaneous term (−0.27, the controller cutting throttle when fast)** and a **positive short-lag term (+0.28, the plant responding)** of near-equal size — they cancel. Binned throttle-vs-ERPM shows throttle varying **8×** with essentially **no** ERPM response. | 🔑 **In closed loop, throttle is a FUNCTION of the error, so regressing speed on throttle recovers the CONTROLLER, not the plant.** Identifying `RO_MAX_THR_SPEED` needs an **open-loop** throttle sweep (RC Manual, stepped throttle, logged ERPM). ⛔ **The 7.4× claim is RETRACTED — do not act on it** |
| 🔴🔴 **`si_motor_poles` and `erpm_to_ms` are a LINKED PAIR — changing one silently breaks odometry 2×** | `canard_driver.c:494` divides the reported RPM by **each VESC's own flashed `si_motor_poles`**. That value is currently set to **half the true pole count** (pole-**pairs** entered where the pole **count** belongs), giving R = 2. `erpm_to_ms` has been silently absorbing it and lands on the right answer. | ⛔ **"Fixing" `si_motor_poles` to the true count in VESC Tool HALVES the reported rpm and makes `/odom` read 2× short — with no error logged anywhere.** PX4's speed loop is fed from `/odom` via `rover-ekf-bridge`, so this lands directly in the control and safety path. **Neither value may change without the other, in the same commit.** Same applies to swapping an ESC or a motor with a different pole count. ✅ **All four ESCs verified on the SAME scale 2026-08-13** (right-front 120.71 sits inside the left-side spread), so `wheel_odometry_node` is averaging like with like |
...
| **Standoff ≥300 mm from a WALL** | Drive at an obstacle, `/scan` healthy | Settled bumper clearance ≥ 0.30 m | ✅ **PASSED 2026-08-12, n=3, and TAPE-CONFIRMED** — `tools/collision_standoff_test.py`. Settled **0.315 / 0.316 / 0.342 m** by `/scan`; the third taped **0.345 m**. Trigger fires within **1–6 mm** of the 0.350 m spec every time. ⚠️ **The pass carries a SPEED BOUND**: at ~0.11 m/s it leaves 0.345 m — ⚠️ **but that 0.11 was `/odom`-derived, and `/odom` under-reads ~21% at crawl (confirmed 2026-08-13), so the true speed was probably ~0.14 m/s. NOT re-measured; the standoff distance itself was taped and is unaffected, but the speed it is conditional on is understated**; at **~0.9 m/s it left 0.020 m and the rover CONTACTED the wall** (2026-08-12). The threshold is not what sets standoff — **the coast is**, and the coast scales with speed |
...
| **Speed tracking at CRAWL** | 🔴 **BROKEN — but the CONTROLLER is not the culprit (2026-08-12/13)** | Commanded **0.060 m/s**, actual **~0.146 m/s** — **2.4×**. Instrumented re-test at **0.050** gave **0.140 m/s = 2.80×** (`tools/speed_command_test.py`, 8.1 s, 3301 samples, armed, floor, EKF bridge up). 🔑 **The speed loop is working and losing**: no throttle saturation (0/3301), slope −0.0014/s (windup predicts +0.0050/s), throttle held at **0.032 vs a 0.083 feedforward**. So the loop *detected* the overspeed and cut throttle by 2.6× — and the rover still ran 2.8× the command. ⇒ the fault is **downstream** (allocation / ESC / `erpm_to_ms`), not in the PID. There is a floor near **0.14 m/s**. ⚠️ **PX4's feedback is CIRCULAR**: `rover-ekf-bridge` feeds the EKF from `/odom`, so the controller inherits `/odom`'s under-read and regulates to a speed **~21% faster than it believes**. That sits in the safety path. ⛔ **You cannot buy standoff margin by slowing down** |
| **`/odom` scale is SPEED-DEPENDENT** | 🔴 **CONFIRMED 2026-08-12 — this is the answer to the 17%** | Two runs at the same wall, ~8× apart in speed: at **~0.9 m/s `/odom` over-reads +5%**; at **~0.11 m/s it under-reads −22%** (−22.3% after the `/scan` scale correction), reproduced twice. One mechanism fits: **wheel slip rising with speed.** Implied constant **0.00366 at speed** vs **~0.0049–0.0051 at crawl**. The fast figure sits beside the configured **0.003900**, which is why `house_map_v4` — built at normal driving speed — taped true. 🔑 **`erpm_to_ms` cannot be "fixed" with a new single number**: any correction must be speed-aware, or it breaks the regime the map was built in. ⚠️ The crawl figure **exceeds the geometric 0.004633**, which slip cannot do — so either the assumed 0.1524 m wheel diameter is small or the 516.7 ERPM-s/5-rev figure is off. **Unexplained; do not fit a story to it**. ✅ **THIRD MEASUREMENT 2026-08-12/13 agrees**: during the speed-controller run `/odom` read **0.111 m/s** against a bumper-derived **0.140 m/s** — **under-reads 20.9%** at crawl, a third independent reproduction of the ~22%, and this time the ruler itself was tape-validated to 1 mm afterwards. ⚠️ **`/odom` also LOOKS like stick-slip at crawl and is not**: it swung with CoV **0.30** and dipped to 0.070 while the bumper showed steady 0.13–0.15 (CoV 0.124). **ERPM is coarsely quantised at low rpm** (observed values 1, 5, 7, 8, 11, 12 …) and `/odom` inherits every step. 🔑 **Never diagnose a mechanical fault from `/odom` velocity at crawl — I nearly recorded a drivetrain stick-slip fault that does not exist**. ✅ **FOURTH MEASUREMENT 2026-08-13, the cleanest yet** — `tools/wheel_erpm_log.py`, RC **Manual** (open loop, no arming by the companion, no EKF bridge), 21.97 s, **100% coverage, 0 gaps**, operator taped both ends (1.973 → 0.042 m). Travel **1.931 m taped vs 1.928 m by `/scan`** — 3 mm. **`erpm_to_ms` = 1.931 / 450.5 ERPM-s = 0.004286** at **0.088 m/s** ⇒ `/odom` under-reads **9.2%**. ⚠️ **THIS CONTRADICTS THE SLIP STORY**: 0.088 m/s → −9.2% but 0.140 m/s → −20.9% — **SLOWER gave LESS under-read**, the opposite of slip rising with speed. The runs differ in more than speed (Manual open-loop vs AutoNav closed-loop). **Two points; do not fit a curve to them.** 🔬 **DEADBAND HYPOTHESIS DISPROVEN**: 50% of per-wheel samples read under the 5-ERPM deadband and it costs the integral **0.37%** — zeroing a wheel that reads 2 removes 2, so the deadband is *arithmetically* incapable of a 20% error. ⛔ Don't re-propose it. 🔴 **THE REAL SIGNAL DEFECT: 46% of samples read EXACTLY 0 ERPM while the rover was provably moving**, near-identically on all four wheels (46.2/46.7/46.4/46.4%, side asymmetry 1.3%) ⇒ the **ESC's ERPM reporting drops out at low speed**; not a wheel fault. 🔴🔴 **PROMOTED TO ROOT CAUSE 2026-08-13 — this is the WHOLE crawl error, and the scale is innocent.** With the corrected slip-free constant (§5), `erpm_to_ms` accounts for **~0%** of the 23.3% crawl under-read and the dropout for **~24%**. ⛔ **Stop looking at the scale constant; this is the defect.** 🔑 **NEW LEAD — severity varies PER ESC, not just with speed:** hand-spun at ~1 rev/s, addr 10 dropped only **31.4%** zeros while addr 11/13 dropped **42–57%** — and addr 10 was spun *slower* (mean 96.6 vs 103–133 rpm), so a pure low-speed effect predicts the opposite. ⚠️ **Distinguish GENUINE STILLNESS from dropout before quoting a zero fraction**: in the 2026-08-13 hand spins most zeros were real pauses, and interpolating across the true dropouts changed the integral by only **2.4%**. A raw "47.9% zeros" figure badly overstates the loss. ⛔ **`tools/odom_scale_measure.py` MUST NOT be used at crawl** — its deadband is **40 on the COMBINED mean** vs the node's **5 PER WHEEL** applied at a different stage (its docstring claims they match; they do not), and it would have eaten **15.27%** of this run |
