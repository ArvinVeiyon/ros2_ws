...
| **Standoff ≥300 mm from a WALL** | Drive at an obstacle, `/scan` healthy | Settled bumper clearance ≥ 0.30 m | ✅ **PASSED 2026-08-11 by 15 mm** — `tools/collision_standoff_test.py`, n=1. Fired at **0.347 m** (vs `stop_distance` 0.35 — accurate to 3 mm), coast **0.034 m**, settled **0.315 m**. **No tape, and none needed here:** the *raw* camera-to-wall range was **0.652 m** — mid-range for the 336L, far from the 0.308 m depth minimum — against a flat perpendicular wall, the sensor's ideal target; and `front_overhang` 0.337 m is itself measured (zero gap, 178 scans, min==max). ⚠️ Margin is only 15 mm and n=1 |
...
| **`/odom` distance vs `/scan` closure** | 🔴 **17% DISAGREEMENT — and `/scan` is the trusted ruler, so suspicion falls on `/odom`** | Same motion, two rulers: `/scan` closure **1.543 m** (a *differential* measurement, independent of `front_overhang`) vs `/odom` pose displacement **1.288 m**. The depth reading was taken mid-range against a flat wall (raw 0.65–0.68 m), i.e. under conditions the 336L is good at ⇒ **the likely fault is the wheel-odometry scale**, not the camera. That is close to the *old* 19% error which `erpm_to_ms → 0.003900` is recorded as having cut to ~2.4% — **this contradicts that record.** 🔑 **`house_map_v4` was built on wheel odometry**, so if the scale is off the map's scale is off, and every M3 plan built on it inherits it. **Settle with `tools/odom_scale_measure.py`** (computes the scale from raw `esc_status`, independent of the suspect value; RC/Manual open-loop) |
