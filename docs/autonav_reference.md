...
| **`/odom` distance vs `/scan` closure** | ⚠️ **17% DISAGREEMENT, REAL BUT NOT A SCALE ERROR — cause still unknown** | Same motion, two rulers: `/scan` closure **1.543 m** vs `/odom` pose displacement **1.288 m** (2026-08-11, crawl). **I initially read this as the odometry scale being wrong and flagged it as contradicting the `erpm_to_ms` record. That inference is WITHDRAWN** — see the map-scale row. The scale is right; whatever produced this is specific to that run, most likely to its very low speed. Candidates not yet tested: low-ERPM integration or dropped samples at crawl (the node's `deadband_erpm` is only 5.0, so the deadband is **not** it — checked), or a flaw in the comparison itself. **`tools/odom_scale_measure.py` at a brisk speed AND at a crawl** would show whether the scale is speed-dependent |
